...
def main():
    ...
    def compute_metrics(p: EvalPrediction, threshold=0.7):
        """
        CoLA 데이터셋 평가를 위한 MCC 계산
        """
        preds = p.predictions[0] if isinstance(p.predictions, tuple) else p.predictions
        preds = np.squeeze(preds)  # 회귀 출력값 (실수 값)

        # Threshold 적용 (이진 분류)
        binary_preds = (preds >= threshold).astype(int)
        binary_labels = p.label_ids.astype(int)
    ...
    trainer = Trainer(
        ...
        compute_metrics=lambda p: compute_metrics(p, threshold=0.7),
